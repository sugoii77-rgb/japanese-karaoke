...
interface WordResult {
  word: string;
  reading: string;
  meanings: { english: string; partsOfSpeech: string }[];
  jlpt: string;
  common: boolean;
}
...
export default function WordPopup({ word, onClose, onAddVocab, onReadFrom }: WordPopupProps) {
  ...
  const posToKorean = useCallback((pos: string) => {
    if (!pos) return '';
  ...
  const meaningToKorean = useCallback((clickedWord: string, english: string) => {
    if (!english) return '';
  ...
  const handleAddVocab = () => {
    if (results.length > 0) {
      const r = results[0];
      const meaning = r.meanings[0]
        ? meaningToKorean(r.word || word, r.meanings[0].english)
        : '';
                ...
                {result.meanings.map((m, j) => (
                  <div key={j} style={{ marginBottom: '6px' }}>
                    {m.partsOfSpeech && (
                      ...
                      >
                        {posToKorean(m.partsOfSpeech)}
                      </span>
                    )}
                    <span style={{ fontSize: '14px', color: '#3d2b1f' }}>
                      {meaningToKorean(result.word || word, m.english)}
                    </span>
                  </div>
                ))}
              </div>
